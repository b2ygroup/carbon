// lib/screens/dashboard_screen.dart (COMPLETO - Revisão Final dos Erros Anteriores)
import 'dart:async';
...
class _DashboardScreenState extends State<DashboardScreen> with SingleTickerProviderStateMixin {
  ...
  // --- Navegação ---
  void _navigateToAddVehicle() { Navigator.of(context).push(MaterialPageRoute(builder: (ctx) => RegistrationScreen())); }
  ...
  Widget _buildIndicatorsSection(String userId) {
      ...
      final Color walletColor = Colors.amberAccent[100]!;
      final Color errorColor = Colors.redAccent[100]!;

      // <<< CORRIGIDO: StreamBuilder externo completo >>>
      return StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('trips').where('userId', isEqualTo: userId).snapshots(), // <<< stream: OK
        builder: (context, tripSnapshot) { // <<< builder: OK
          double totalKm = 0.0; double totalCO2 = 0.0; double totalCredits = 0.0;
          bool tripsHaveError = tripSnapshot.hasError;
          bool tripIndicatorsLoading = tripSnapshot.connectionState == ConnectionState.waiting;
          if (!tripsHaveError && tripSnapshot.hasData) {
            for (var doc in tripSnapshot.data!.docs) {
              final data = doc.data() as Map<String, dynamic>? ?? {};
              totalKm += (data['distanceKm'] as num?)?.toDouble() ?? 0.0;
              totalCO2 += (data['co2SavedKg'] as num?)?.toDouble() ?? 0.0;
              totalCredits += (data['creditsEarned'] as num?)?.toDouble() ?? 0.0;
            }
          }
          if(tripsHaveError) print("Erro trip stream: ${tripSnapshot.error}");

          // <<< CORRIGIDO: StreamBuilder interno completo >>>
          return StreamBuilder<DocumentSnapshot>(
            stream: FirebaseFirestore.instance.collection('wallets').doc(userId).snapshots(), // <<< stream: OK
            builder: (context, walletSnapshot) { // <<< builder: OK
              double walletBalance = 0.0;
              bool walletHasError = walletSnapshot.hasError;
              bool walletIsLoading = walletSnapshot.connectionState == ConnectionState.waiting;
              String walletValueForDisplay = "...";
              if (!walletIsLoading && !walletHasError && walletSnapshot.hasData) {
                 if (walletSnapshot.data!.exists) {
                    final d = walletSnapshot.data!.data() as Map<String, dynamic>?;
                    if (d != null && d.containsKey('balance')) {
                       walletBalance = (d['balance'] as num?)?.toDouble() ?? 0.0;
                       walletValueForDisplay = "R\$ ${walletBalance.toStringAsFixed(2)}";
                    } else { walletHasError = true; walletValueForDisplay = "Inválido"; }
                 } else { walletValueForDisplay = "R\$ 0.00"; }
              } else if (walletHasError) { walletValueForDisplay = "Erro"; print("Erro wallet stream: ${walletSnapshot.error}"); }

              // <<< CORRIGIDO: Padding externo completo >>>
              return Padding(
                padding: const EdgeInsets.symmetric(horizontal: 60.0), // <<< padding: OK
  ...
  Widget _buildProgressBarSection() {
    double currentProgress = 0.75; // Exemplo
    // <<< CORRIGIDO: Padding completo >>>
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 24.0), // <<< padding: OK
      child: Row(
        children: [
          // <<< CORRIGIDO: CircularPercentIndicator completo >>>
          CircularPercentIndicator(
            radius: 35.0, // <<< radius: OK
            lineWidth: 8.0,
            percent: currentProgress,
            center: Text(
              "${(currentProgress * 100).toInt()}%",
              style: GoogleFonts.orbitron(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.white70),
            ),
  ...
  Widget _buildActionButtons() {
    ...
    final Color unselectedColor = Colors.grey[850]!;
    final Color selectedTextColor = Colors.black87;
    final Color unselectedTextColor = Colors.white70;

    // <<< CORRIGIDO: Padding completo >>>
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 16.0), // <<< padding: OK
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: [
           // <<< CORRIGIDO: ElevatedButton.icon completo >>>
           Expanded(
             child: ElevatedButton.icon(
               icon: const Icon(Icons.gps_fixed_rounded, size: 18), // <<< icon: OK
               label: const Text("Monitorar GPS"), // <<< label: OK
               onPressed: () { // <<< onPressed: OK
                  if(_tabController.index != 0) _tabController.animateTo(0);
               },
               style: ElevatedButton.styleFrom(
                 backgroundColor: _tabController.index == 0 ? selectedColor : unselectedColor,
                 foregroundColor: _tabController.index == 0 ? selectedTextColor : unselectedTextColor,
                 shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                 padding: const EdgeInsets.symmetric(vertical: 12),
                  side: _tabController.index != 0 ? BorderSide(color: Colors.grey[700]!) : null
               ),
             ),
           ),
           ...
           Expanded(
             child: ElevatedButton.icon(
               icon: const Icon(Icons.calculate_rounded, size: 18), // <<< icon: OK
               label: const Text("Calcular Rota"), // <<< label: OK
               onPressed: () { // <<< onPressed: OK
                  if(_tabController.index != 1) _tabController.animateTo(1);
               },
               style: ElevatedButton.styleFrom(
                 backgroundColor: _tabController.index == 1 ? selectedColor : unselectedColor,
                 foregroundColor: _tabController.index == 1 ? selectedTextColor : unselectedTextColor,
                 shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                 padding: const EdgeInsets.symmetric(vertical: 12),
                 side: _tabController.index != 1 ? BorderSide(color: Colors.grey[700]!) : null
               ),
             ),
           ),
        ],
     ...
     return Card(
       ...
       child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Monitorar Viagem GPS', style: GoogleFonts.orbitron(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
              ...
              ListTile(
                ...
                leading: Icon(
                  ...
                  color: _selectedVehicleIdForTrip != null
                      ? (_selectedVehicleTypeForTrip?.displayColor ?? accentColor)
                      : subtleTextColor,
                  size: 32,
                ),
                ...
                subtitle: Text(
                   _selectedVehicleIdForTrip != null
                    ? '${_selectedVehicleTypeForTrip?.displayName ?? 'Tipo Desconhecido'} (Toque na lista abaixo para alterar)'
                    : "Selecione um veículo na lista abaixo",
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  labelText: 'Origem (Opcional)', labelStyle: TextStyle(color: subtleTextColor),
                  hintText: 'Ex: Casa, Trabalho...', hintStyle: TextStyle(color: subtleTextColor.withOpacity(0.5)),
                  isDense: true, prefixIcon: Icon(Icons.trip_origin, color: subtleTextColor),
              ...
              TextFormField(
                ...
                enabled: !_isTracking,
                 style: const TextStyle(color: Colors.white),
                decoration: InputDecoration(
                  labelText: 'Destino (Opcional)', labelStyle: TextStyle(color: subtleTextColor),
                  hintText: 'Ex: Mercado, Academia...', hintStyle: TextStyle(color: subtleTextColor.withOpacity(0.5)),
                  isDense: true, prefixIcon: Icon(Icons.flag_outlined, color: subtleTextColor),
                   filled: true, fillColor: Colors.black.withOpacity(0.2),
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
                  focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: accentColor)),
                ),
              ...
              const SizedBox(height: 20),
              if (_isTracking) Center( child: Padding( padding: const EdgeInsets.symmetric(vertical: 8.0), child: Row( mainAxisAlignment: MainAxisAlignment.center, children: [ Icon(Icons.route, color: accentColor, size: 26), const SizedBox(width: 8), Text( '${_currentDistanceKm.toStringAsFixed(2)} km', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: accentColor) ), const SizedBox(width: 10), SpinKitPulse(color: accentColor, size: 15.0) ] ) ) ).animate(onPlay: (c)=>c.repeat()).shimmer(delay: 400.ms, duration: 1000.ms, color: Colors.white.withOpacity(0.1)).scaleY(),
              const SizedBox(height: 20),
              Center(
                child: _isLoadingGpsSave
                  ? Padding( padding: const EdgeInsets.symmetric(vertical: 10), child: Column( children: [ SpinKitWave(color: accentColor, size: 25.0), const SizedBox(height: 8), Text("Salvando viagem...", style: TextStyle(color: subtleTextColor)) ], ) )
                  : ElevatedButton.icon(
                      ...
                      label: Padding( padding: const EdgeInsets.symmetric(vertical: 12.0), child: Text(_isTracking ? 'Parar e Salvar Viagem' : 'Iniciar Viagem', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), ),
                      style: ElevatedButton.styleFrom( backgroundColor: _isTracking ? errorColor.withOpacity(0.9) : accentColor, foregroundColor: _isTracking ? Colors.white : Colors.black87, padding: const EdgeInsets.symmetric(horizontal: 30, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)), elevation: 8, shadowColor: _isTracking ? errorColor : accentColor, ),
  ...
  Widget _buildVehicleSectionHeader(ThemeData theme, Color primaryColor) {
     ...
     final buttonColor = Colors.cyanAccent[400]!;
     // <<< CORRIGIDO: Padding com padding >>>
     return Padding(
       padding: const EdgeInsets.only(top: 16.0), // <<< padding: OK
       child: Row(
          ...
          children: [
            ...
            Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                 TextButton.icon( icon: Icon(Icons.add_circle_outline, size: 18, color: buttonColor), label: Text('Adicionar', style: TextStyle(color: buttonColor.withOpacity(0.8), fontSize: 13)), style: TextButton.styleFrom( padding: const EdgeInsets.symmetric(horizontal: 8), ), onPressed: _navigateToAddVehicle ),
                 const SizedBox(width: 0),
                 TextButton.icon( icon: Icon(Icons.list_alt_rounded, size: 18, color: Colors.grey[500]), label: Text('Gerenciar', style: TextStyle(color: Colors.grey[500], fontSize: 13)), style: TextButton.styleFrom( padding: const EdgeInsets.symmetric(horizontal: 8), ), onPressed: _navigateToFleetManagement ),
              ],
            )
          ],
        ),
  ...
  Widget _buildVehicleList(String userId, ThemeData theme, Color primaryColor, Color subtleTextColor) {
     final cardColor = Colors.grey[850]!.withOpacity(0.6);
     final selectedBorderColor = Colors.cyanAccent[400]!;
     final normalBorderColor = Colors.grey[700]!;
     final titleColor = Colors.white.withOpacity(0.9);
     final subtitleColor = Colors.white.withOpacity(0.6);
     final iconColor = Colors.white70;
     final accentColor = Colors.cyanAccent[400]!;

     // <<< CORRIGIDO: StreamBuilder completo >>>
     return StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('vehicles').where('userId', isEqualTo: userId).orderBy('createdAt', descending: true).snapshots(), // <<< stream: OK
        builder: (ctx, snapshot) { // <<< builder: OK
          if (snapshot.connectionState == ConnectionState.waiting) { return const Center(child: Padding(padding: EdgeInsets.symmetric(vertical: 30.0), child: CircularProgressIndicator(strokeWidth: 2, color: Colors.cyanAccent))); }
          if (snapshot.hasError) { return Center(child: Text('Erro ao carregar veículos.', style: TextStyle(color: theme.colorScheme.error))); }
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) { /* ... Card Nenhum Veículo ... */ }

          final vehicleDocs = snapshot.data!.docs;
          // <<< CORRIGIDO: ListView.separated completo >>>
          return ListView.separated(
             shrinkWrap: true,
             physics: const NeverScrollableScrollPhysics(),
             itemCount: vehicleDocs.length, // <<< itemCount: OK
             separatorBuilder: (context, index) => const SizedBox(height: 8), // <<< separatorBuilder: OK
             itemBuilder: (ctx, index) { // <<< itemBuilder: OK (conteúdo interno completo)
                final vehicleData = vehicleDocs[index].data() as Map<String, dynamic>;
                final vehicleId = vehicleDocs[index].id;
                final vehicleType = vehicleTypeFromString(vehicleData['type']);
                final isSelected = vehicleId == _selectedVehicleIdForTrip;
                ...
                return Card(
                   ...
                   shape: RoundedRectangleBorder(
                      ...
                      side: BorderSide( color: isSelected ? selectedBorderColor : normalBorderColor, width: isSelected ? 2.0 : 0.8, )
                   ),
                   child: InkWell(
                      onTap: () => _handleVehicleSelection(vehicleId, vehicleType),
                      borderRadius: BorderRadius.circular(12),
                      ...
                      child: Padding(
                        ...
                        child: ListTile(
                            dense: true,
                            contentPadding: EdgeInsets.zero,
                            leading: Icon( vehicleType?.icon ?? Icons.help_outline, color: isSelected ? accentColor : (vehicleType?.displayColor ?? iconColor), size: 28, ),
                            title: Text( '${vehicleData['make'] ?? '?'} ${vehicleData['model'] ?? '?'}', style: TextStyle( fontWeight: isSelected ? FontWeight.bold : FontWeight.w500, fontSize: 15, color: titleColor ) ),
                            subtitle: Text( '${vehicleData['year'] ?? '?'} - ${vehicleData['licensePlate'] ?? 'Sem placa'}\nTipo: ${vehicleType?.displayName ?? vehicleData['type'] ?? '?'}', style: TextStyle(color: subtitleColor, fontSize: 12.5, height: 1.3) ),
                            trailing: isSelected ? Icon(Icons.check_circle, color: selectedBorderColor, size: 22) : Icon(Icons.radio_button_unchecked, color: subtitleColor.withOpacity(0.5), size: 20),
                        ),
                      ),
                   ),
                ).animate().fadeIn(delay: (50 * index).ms).slideX(begin: 0.1);
             }, // Fim itemBuilder
          ); // Fim ListView.separated
       ...
       child: Wrap(
          ...
          children: [
            ...
            ElevatedButton.icon(
                ...
                icon: Icon(Icons.history, size: 18, color: iconColor), // <<< icon: OK
                label: const Text('Histórico', style: TextStyle(fontSize: 13)), // <<< label: OK
                onPressed: _navigateToTripHistory), // <<< onPressed: OK
            // <<< CORRIGIDO: ElevatedButton.icon completo >>>
            ElevatedButton.icon(
                 style: ElevatedButton.styleFrom( backgroundColor: buttonColor, foregroundColor: textColor, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)) ),
                icon: Icon(Icons.account_balance_wallet, size: 18, color: iconColor), // <<< icon: OK
                label: const Text('Carteira', style: TextStyle(fontSize: 13)), // <<< label: OK
                onPressed: () {/*TODO: Navegar Carteira*/}), // <<< onPressed: OK
            // <<< CORRIGIDO: ElevatedButton.icon completo >>>
            ElevatedButton.icon(
                style: ElevatedButton.styleFrom( backgroundColor: buttonColor, foregroundColor: textColor, padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)) ),
                icon: Icon(Icons.store, size: 18, color: iconColor), // <<< icon: OK
                label: const Text('Mercado', style: TextStyle(fontSize: 13)), // <<< label: OK
                onPressed: () {/*TODO: Navegar Mercado*/}), // <<< onPressed: OK
          ],
  ...
  Widget _buildScrollableTabContent(Widget tabSpecificContent) {
     ...
     final subtleTextColor = Colors.white.withOpacity(0.6);
     final user = _currentUser;
     if (user == null) return const Center(child: Text("Erro: Usuário não encontrado."));
     final String userId = user.uid;

     return ListView(
        padding: const EdgeInsets.fromLTRB(16.0, 20, 16.0, 80.0),
        children: [
           // Ad Banner
           const AdBannerPlaceholder(), // Movido para cá
           const SizedBox(height: 20),
           ...
           Divider(height: 30, thickness: 0.5, color: Colors.grey[800]), // Mais espaço com divider
           ...
           Text('Desempenho Recente', style: GoogleFonts.orbitron(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white.withOpacity(0.9))),
           const SizedBox(height: 16),
            ...
            return <Widget>[
              SliverAppBar(
                       ...
                       child: Column(
                         ...
                         children: [
                           Row(
                             mainAxisAlignment: MainAxisAlignment.spaceBetween,
                             children: [
                               Text('B2Y Carbon Cockpit', style: GoogleFonts.orbitron(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.white.withOpacity(0.95))),
                               IconButton(icon: Icon(Icons.power_settings_new_rounded, color: accentColor), tooltip: 'Sair', onPressed: _logout)
                             ],
                           ),
                           const SizedBox(height: 4),
                           Text( 'Olá, $displayName!', style: GoogleFonts.poppins(textStyle: theme.textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w500, color: Colors.white.withOpacity(0.8))) ),
                         ],
                       ),
                     ),
                   ),
                ...
                bottom: TabBar(
                  ...
                  indicatorColor: accentColor, indicatorWeight: 3.0, labelColor: accentColor, unselectedLabelColor: Colors.grey[600],
                  labelStyle: GoogleFonts.rajdhani(fontWeight: FontWeight.bold, fontSize: 13), unselectedLabelStyle: GoogleFonts.rajdhani(fontSize: 13),
                  tabs: const [ Tab(text: 'MONITORAR'), Tab(text: 'CALCULAR'), ],
                ),
              ),
              // Colocando os botões de ação aqui para ficarem fixos abaixo da TabBar
              SliverToBoxAdapter(child: _buildActionButtons()),
              SliverToBoxAdapter(child: Divider(height: 1, thickness: 0.5, color: Colors.grey[800], indent: 16, endIndent: 16)), // Divisor após botões

            ];
          },
          // O corpo agora só precisa da TabBarView
          body: TabBarView(
            controller: _tabController,
            children: [
              // Conteúdo da Aba 0 (Monitorar)
              _buildScrollableTabContent(
                 _buildGpsTrackingTabContent(theme, Colors.grey[500]!, accentColor)
              ),
              // Conteúdo da Aba 1 (Calcular)
              _buildScrollableTabContent(
